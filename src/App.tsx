...
import { AIPromptGeneratorBar } from './components/canvas/AIPromptGeneratorBar';
import { ExportImportModal } from './components/ExportImportModal';
import { ExecutionHistoryDrawer } from './components/canvas/ExecutionHistoryDrawer';
...
export default function App() {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          setNodes((prev) =>
            prev.map((n) =>
              n.id === node.id
                ? { ...n, data: { ...n.data, status: 'success', executionTimeMs: duration } }
                : n
            )
          );
  ...
  const handleImportWorkflow = (importedNodes: WorkflowNode[], importedConns: Connection[], name?: string) => {
    const newWf: Workflow = {
      id: `wf-${Date.now()}`,
      name: name || 'New AI Generated Workflow',
      description: 'Generated or Imported into AutoFlow AI',
      category: 'Custom',
      nodes: importedNodes,
  ...
  return (
    <div className="flex flex-col h-screen w-screen overflow-hidden bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans select-none">
      ...
      <div className="flex-1 relative flex overflow-hidden">
        ...
        {activeView === 'canvas' && (
          <div className="flex-1 flex relative overflow-hidden">
            ...
            <div className="flex-1 relative overflow-hidden">
              ...
              <AIPromptGeneratorBar
                onGenerateWorkflow={(gNodes, gConns, name) => {
                  updateNodesAndConns(gNodes, gConns);
                  setWorkflows((prev) =>
                    prev.map((w) => (w.id === activeWorkflowId ? { ...w, name, updatedAt: 'Just now' } : w))
                  );
                }}
              />
        ...
        {activeView === 'marketplace' && (
          <div className="flex-1 overflow-y-auto">
            <MarketplaceView
              onInstallTemplate={(wf) => handleImportWorkflow(wf.nodes || [], wf.connections || [], wf.name)}
            />
          </div>
        )}
        ...
        {activeView === 'meeting' && (
          <div className="flex-1 overflow-y-auto">
            <MeetingToWorkflowView
              onImportWorkflow={(wf) => handleImportWorkflow(wf.nodes || [], wf.connections || [], wf.name)}
            />
          </div>
        )}
      ...
      <VoiceWorkflowModal
        ...
        onWorkflowGenerated={(wf) => handleImportWorkflow(wf.nodes || [], wf.connections || [], wf.name)}
      />
      ...
      <ExportImportModal
        ...
        nodes={nodes}
        connections={connections}
        onImportWorkflow={(iNodes, iConns, name) => handleImportWorkflow(iNodes, iConns, name)}
      />
